...
const weatherLocation = async (location) => {
  const urlWeather = `https://api.openweathermap.org/data/2.5/weather?${location}&appid=${
    import.meta.env.VITE_API_KEY
  }&units=metric`;
  const urlForecast = `https://api.openweathermap.org/data/2.5/forecast?${location}&appid=${
    import.meta.env.VITE_API_KEY
  }&units=metric`;
  ...
  try {
     ...
     */
    const resolve = await Promise.all([fetch(urlWeather), fetch(urlForecast)]);
...
<template>
  <div class="min-h-screen flex flex-col md:max-w-5xl mx-auto p-4 font-albert">
    ...
    <main class="flex flex-grow md:justify-center">
      <div class="flex flex-col sm:flex-row w-full gap-4" v-if="weather">
