<template>
  ...
  >
    ...
    >
      <LineChart :chartData="tempData" :options="options" style="height: 25vh; width: 80vw" />
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  setup(props) {
    const listDay = computed(() => [...props.forecast.map((item) => getWeekDay(item.dt_txt))])
      ...
      return {
        ...
        datasets: [
          ...
          },
          {
            ...
            data: props.forecast.map((item) => Math.round(item.main.feels_like)),
            radius: 5,
            borderWidth: 4,
            borderColor: ['#51BDF8'],
            backgroundColor: ['white'],
